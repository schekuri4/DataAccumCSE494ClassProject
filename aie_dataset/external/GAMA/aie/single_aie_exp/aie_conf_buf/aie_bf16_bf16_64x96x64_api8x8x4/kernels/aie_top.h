...
PATH: aie/single_aie_exp/aie_conf_buf/aie_bf16_bf16_64x96x64_api8x8x4/kernels/aie_top.h
DOMAIN: AIE Source
INTERFACE: PLIO/GMIO
KEY INTRINSICS: connect, dimensions, runtime, location
...
public:
    ...
    mm_graph() {
        ...
        location<buffer>(mm[0].in[0]) = location<kernel>(mm[0]);

        location<buffer>(mm[0].in[1]) = location<kernel>(mm[0]);
        location<buffer>(mm[0].out[0]) = location<kernel>(mm[0]);
    }
};