...
PATH: AI_Engine_Development/AIE/Design_Tutorials/03-beamforming/Module_02_AI_Engine_Design/src/inc/subsys.h
DOMAIN: DSP — 5G NR Beamforming (Downlink 64-antenna 32-layer / Uplink 64-antenna 32-layer)
INTERFACE: Window (connect with explicit dimensions), cascade stream (connect<cascade>)
KEY PATTERNS:
  - Template graph class parameterized by tile coordinates <xoff, yoff, len>
  - connect<cascade>() for cascade stream chaining between adjacent tiles
  - location<kernel>() / location<stack>() / location<buffer>() placement constraints
  - bank() constraints for ping-pong buffer placement in specific memory banks
  - Hierarchical graph composition: bfCascadeChain → DL64A32L / UL64A32L
  - runtime<ratio>(core[i]) = 0.8 for scheduling
  - initialization_function() for persistent state initialization
VECTOR TYPES: cint16 (complex 16-bit int), cacc48 (complex 48-bit accumulator)
TILE COVERAGE: DL64A32L uses 2×4 tile blocks; UL64A32L uses 1×4 tile blocks of 8 kernels each
*/
