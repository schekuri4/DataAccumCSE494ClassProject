...
PATH: AI_Engine_Development/AIE/Feature_Tutorials/09-debug-walkthrough/cmd_src/aie/kernels/data_shuffle.cc
DOMAIN: Debug / Stream correctness
INTERFACE: Window (input_buffer / output_buffer) + Stream (input_stream)
KEY INTRINSICS: aie::begin_vector, aie::zeros, readincr, aie::broadcast, aie::shuffle_up_fill
VECTOR TYPES: aie::vector<int32,8>
DEBUG NOTE: This is the CORRECT version. The BUG version has the readincr(outmax) call
            removed (line with "max_scalar = readincr(outmax);"), causing a stream deadlock.
            Symptom: Lock_East stall detected at T=575600.000000 ps in aiesimulator.
            Fix: Ensure the stream consumer reads exactly as many tokens as the producer writes.
*/
...
)
{
	...
	for(int i=0;i<NUM_SAMPLES*2;i++)
	{
		...
		if(remainder == 0)				                       //previous kernel sends 1 output for every 16 values
			max_scalar = readincr(outmax);                             // <-- CRITICAL: removing this line causes deadlock
