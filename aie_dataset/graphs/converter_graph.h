...
PATH: AI_Engine_Development/AIE/Feature_Tutorials/09-debug-walkthrough/cmd_src/aie/graph.h
DOMAIN: Debug walkthrough — multi-kernel ADF graph
INTERFACE: PLIO (plio_64_bits), Window buffers
TOPOLOGY: 1 input PLIO → peak_detect → upscale → output PLIO
                       → data_shuffle → output PLIO
          peak_detect connects to BOTH upscale and data_shuffle via shared output port
          Cascade-like pattern using stream (out[1]) shared across two downstream kernels
KEY PATTERNS:
  - input_plio / output_plio creation
  - adf::kernel::create()
  - adf::source() assignment
  - runtime<ratio>() scheduling
  - connect() port wiring
  - dimensions() buffer sizing
*/
