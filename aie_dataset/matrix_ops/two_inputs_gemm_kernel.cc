...
PATH: designs/xgemm-gmio/aie/kernels/two_inputs_aie.cc
DOMAIN: Matrix Operations — GEMM inner tile kernel
INTERFACE: Window (input_window<int32>, output_window<int32>) + Cascade (input_cascade<acc48>, output_cascade<acc48>)
KEY INTRINSICS: window_readincr_v, window_writeincr, readincr_v, writeincr,
                aie::load_v, aie::store_unaligned_v, aie::mul, aie::reduce_add
                window_acquire / window_release (explicit async window management)
VECTOR TYPES: aie::vector<int32, VECTOR_LENGTH>, aie::accum<acc48, VECTOR_LENGTH>, aie::accum<acc80, VECTOR_LENGTH>
CHESS PRAGMAS: chess_prepare_for_pipelining
SPECIAL: get_coreid() used for dynamic tile position detection (row parity for data flow direction)
         Serpentine data flow: even rows go left-to-right, odd rows go right-to-left
*/
