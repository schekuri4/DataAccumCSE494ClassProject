...
SOURCE: Xilinx/plnx-aie-examples
PATH: designs/xgemm-gmio/aie/xgemm.h
DOMAIN: Matrix Operations — General Matrix Multiply (GEMM)
INTERFACE: GMIO (input_gmio / output_gmio) + Window (connect<window<WIN_SIZE_BYTES>>)
           + Cascade stream (connect<cascade>)
KEY PATTERNS:
  - input_gmio / output_gmio for DDR memory-mapped DMA access (no PLIO)
  - 2D kernel array: krnl[NUM_HW_ROWS][NUM_HW_COLS]
  - location<kernel>() = tile(col, row) for explicit placement
  - connect<window<WIN_SIZE_BYTES>> with async() for non-blocking window handoff
  - connect<cascade> for accumulator streaming between tiles
  - Row-direction reversal for odd rows (serpentine data flow pattern)
  - Dual architecture support: __AIE_ARCH__ == AIE vs AIE-ML
  - kernel::create() with different source files per position (OneInput/TwoInputs/OneOutput)
TILE LAYOUT: NUM_HW_ROWS rows x NUM_HW_COLS columns
*/
